...
canada <- left_join(canada,popdata, by=c("DGUID"="DGUID")) 
...
ui <- fluidPage(
  checkboxGroupInput(inputId = "province", 
                     ...
                     selected = pnames_simp[9,2], 
                     inline = F),
  textOutput("test"),
  tmapOutput(outputId = "canadamap")
)
server <- function(input,output){
  output$test <- renderText(input$province)
  output$canadamap <- renderTmap({tm_shape(st_make_valid(canada[as.numeric(canada$PRUID)%in%input$province,]))+
